...
namespace ManPowerCore.Controller
{
	public interface VehicleMaintenanceController
	{
		...
		int UpdateRecommandationTOStatus(int id, int approvalStatus, string FileNo, int officerID, string reason);

		int UpdateRecommandationStatus(int id, int approvalStatus, int officerID, string reason);

		int UpdateRecommandationADStatus(int id, int approvalStatus, int officer, string reason);
	...
	public class VehicleMaintenanceControllerImpl : VehicleMaintenanceController
	{
		...
		public int UpdateRecommandationTOStatus(int id, int approvalStatus, string FileNo, int officerID, string reason)
		{
			try
			{
				dBConnection = new DBConnection();
				int result = vehicleMaintenanceDAO.UpdateRecommandationTOStatus(id, approvalStatus, FileNo, officerID, reason, dBConnection);
				return result;
			}
			catch (Exception)
			{
		...
		public int UpdateRecommandationADStatus(int id, int approvalStatus, int officerID, string reason)
		{
			try
			{
				dBConnection = new DBConnection();
				int result = vehicleMaintenanceDAO.UpdateRecommandationADStatus(id, approvalStatus, officerID, reason, dBConnection);
				return result;
			}
			catch (Exception)
			{
