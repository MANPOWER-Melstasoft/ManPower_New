...
namespace ManPowerCore.Infrastructure
{
	public interface TrainingMainDAO
	{
		...
		TrainingMain GetTrainingMainById(int id, DBConnection dbConnection);
	}
	...
	public class TrainingMainDAOSqlImpl : TrainingMainDAO
	{
		public int Save(TrainingMain trainingMain, DBConnection dbConnection)
		{
			...
			dbConnection.cmd.CommandText = "INSERT INTO Training_Main (Title, Content, Created_date, Created_user, Member_Count, Open_date, End_date, Post_img) " +
				"VALUES (@Title, @Content, @CreatedDate, @CreatedUser, @MemberCount, @OpenDate, @EndDate, @PostImg) SELECT SCOPE_IDENTITY()";
		...
		public TrainingMain GetTrainingMainById(int id, DBConnection dbConnection)
		{
			if (dbConnection.dr != null)
				dbConnection.dr.Close();

			dbConnection.cmd.CommandText = "SELECT * FROM Training_Main WHERE ID=" + id + " ";

			dbConnection.dr = dbConnection.cmd.ExecuteReader();
			DataAccessObject dataAccessObject = new DataAccessObject();
			return dataAccessObject.GetSingleOject<TrainingMain>(dbConnection.dr);
		}
	}
}
