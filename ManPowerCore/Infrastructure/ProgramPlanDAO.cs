...
namespace ManPowerCore.Infrastructure
{
    ...
    public class ProgramPlanDAOImpl : ProgramPlanDAO
    {
        ...
        public DataTable getProgramPlan(int DepId, DBConnection dbConnection)
        {
            ...
            dbConnection.cmd.CommandText = "select e.Name AS Program_Name,a.ProgramName,a.Project_Status_Id, x.Id, a.Location,a.Date,a.Approved_Amount, x.Program_Id ," +
                ...
                "( select count(*) FROM Job_Refferals jf where jf.Program_Plan_Id = a.id) as Job_Refferals_count," +
                "( select count(*) FROM Training_Refferals tf where tf.Program_Plan_Id = a.id) as Training_Refferals_count, " +
                "( select count(*) FROM Career_Key_Test_Results cr where cr.Program_Plan_Id = a.id) as Career_Key_Test_count " +
                "from" +
                "(select b.Id, b.Program_Id, b.No_Of_Projects, b.Vote_Number," +
                "b.Period_Type as an, 0 as qt, 0 as ml from Program_Target b " +
                "INNER JOIN Program_Assignee c ON c.Program_Target_Id = b.Id WHERE c.Department_Unit_Possitions_Id = " + DepId + " " +
                "and b.Period_Type = 1 " +
                "union all select b.Id, b.Program_Id, 0 as an, b.No_Of_Projects, b.Vote_Number, b.Period_Type as qt, 0 as ml " +
                "from Program_Target b INNER JOIN Program_Assignee c ON c.Program_Target_Id = b.Id WHERE c.Department_Unit_Possitions_Id = " + DepId + " " +
                "and b.Period_Type = 2 union all select b.Id, b.Program_Id, 0 as an, 0 as qt, b.No_Of_Projects, b.Vote_Number, b.Period_Type as ml " +
                "from Program_Target b INNER JOIN Program_Assignee c ON c.Program_Target_Id = b.Id " +
                "WHERE c.Department_Unit_Possitions_Id = " + DepId + " and b.Period_Type = 3) x " +
                "inner join Program_Plan a on  a.Program_Target_Id = x.Id INNER JOIN Program e ON e.id = x.program_id " +
                "LEFT JOIN Resource_Person_Program_Plan f ON f.Program_Plan_Id = a.Id " +
                "LEFT JOIN (select Program_plan_id, string_agg(Name, ',') as Person," +
                "string_agg(Work_Place, ',') as Work_Places  from resource_person_program_plan " +
                "LEFT JOIN Resource_Person g ON Resource_Person_Program_Plan.Resourse_Person_Id = g.id group by Program_plan_id) g ON f.Program_Plan_Id = g.Program_Plan_Id " +
                ...
                "x.Vote_Number,a.Financial_Source, g.Person, g.Work_Places,a.Id,a.Project_Status_Id,a.ProgramName;";
